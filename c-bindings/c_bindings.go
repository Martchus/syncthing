...
func libst_run_syncthing(configDir string, dataDir string, guiAddress string, guiApiKey string, profilerAddress string, dbMaintenanceInterval int64, dbDeleteRetentionInterval int64, verbose bool, allowNewerConfig bool, skipPortProbing bool, ensureConfigDirExists bool, ensureDataDirExists bool, expandPathsFromEnv bool, resetDeltaIdxs bool) int {
	...
	if guiAddress != "" {
		defaultGuiAddress, hasDefaultGuiAddress := os.LookupEnv("STGUIADDRESS")
		os.Setenv("STGUIADDRESS", guiAddress)
		if hasDefaultGuiAddress {
			defer os.Setenv("STGUIADDRESS", defaultGuiAddress)
		} else {
			defer os.Unsetenv("STGUIADDRESS")
		}
	}
	if guiApiKey != "" {
		defaulApiKey, hasDefaultApiKey := os.LookupEnv("STGUIAPIKEY")
		os.Setenv("STGUIAPIKEY", guiApiKey)
		if hasDefaultApiKey {
			defer os.Setenv("STGUIAPIKEY", defaulApiKey)
		} else {
	...
	if err := syncthing.TryMigrateDatabase(ctx, dbMaintIntDuration); err != nil {
	...
	srv := &http.Server{
		...
		Handler: http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			for _, line := range slogutil.GlobalRecorder.Since(time.Time{}) {
				line.WriteTo(w, slogutil.DefaultLineFormat)
			}
		}),
	}
	go func() {
